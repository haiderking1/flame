...
const agentDirEnvName = "FLAME_CODING_AGENT_DIR";
const startupBenchmarkEnvName = "FLAME_STARTUP_BENCHMARK";
...
Options:
  ...
  --agent-dir <dir>      Use a specific FLAME_CODING_AGENT_DIR for the benchmark run
  --isolated-agent-dir   Use a fresh temporary agent dir instead of the normal one
  --no-offline           Do not force FLAME_OFFLINE=1 / FLAME_SKIP_VERSION_CHECK=1
...
function createBenchmarkEnv(options, isolatedAgentDir) {
	...
	if (options.offline) {
		env.FLAME_OFFLINE = "1";
		env.FLAME_SKIP_VERSION_CHECK = "1";
	}
	return env;
}
