# @earendil-works/flame-agent-core

Stateful agent with tool execution and event streaming. Built on `@earendil-works/flame-ai`.

## Installation

```bash
npm install @earendil-works/flame-agent-core
```
...
```typescript
declare module "@earendil-works/flame-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@earendil-works/flame-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@earendil-works/flame-agent-core";
