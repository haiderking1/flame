...
export class TUI extends Container {
	...
	private showHardwareCursor = process.env.FLAME_HARDWARE_CURSOR === "1";
	private clearOnShrink = process.env.FLAME_CLEAR_ON_SHRINK === "1"; // Clear empty rows when content shrinks (default: off)
	...
	private doRender(): void {
		...
		const debugRedraw = process.env.FLAME_DEBUG_REDRAW === "1";
		const logRedraw = (reason: string): void => {
			if (!debugRedraw) return;
			const logPath = path.join(os.homedir(), ".flame", "agent", "flame-debug.log");
		...
		for (let i = firstChanged; i <= renderEnd; i++) {
			...
			if (!isImage && visibleWidth(line) > width) {
				...
				const crashLogPath = path.join(os.homedir(), ".flame", "agent", "flame-crash.log");
				const crashData = [
		...
		if (process.env.FLAME_TUI_DEBUG === "1") {
