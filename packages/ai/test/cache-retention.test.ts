...
describe("Cache Retention (PI_CACHE_RETENTION)", () => {
	const originalEnv = process.env.FLAME_CACHE_RETENTION;

	beforeEach(() => {
		delete process.env.FLAME_CACHE_RETENTION;
	});

	afterEach(() => {
		if (originalEnv !== undefined) {
			process.env.FLAME_CACHE_RETENTION = originalEnv;
		} else {
			delete process.env.FLAME_CACHE_RETENTION;
		}
	});
	...
	describe("Anthropic Provider", () => {
		...
		it.skipIf(!process.env.ANTHROPIC_API_KEY)("should use 1h cache TTL when PI_CACHE_RETENTION=long", async () => {
			process.env.FLAME_CACHE_RETENTION = "long";
			const model = getModel("anthropic", "claude-haiku-4-5");
			let capturedPayload: any = null;
