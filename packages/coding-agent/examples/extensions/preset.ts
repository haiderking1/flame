 ...
 * - ~/.flame/agent/presets.json (global)
 * - <cwd>/.flame/presets.json (project-local)
 *
...
import type { Api, Model } from "@earendil-works/flame-ai";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/flame-coding-agent";
import { DynamicBorder, getAgentDir } from "@earendil-works/flame-coding-agent";
import { Container, Key, type SelectItem, SelectList, Text } from "@earendil-works/flame-tui";
...
function loadPresets(cwd: string): PresetsConfig {
	...
	const projectPath = join(cwd, ".flame", "presets.json");
...
export default function presetExtension(pi: ExtensionAPI) {
	...
	async function showPresetSelector(ctx: ExtensionContext): Promise<void> {
		...
		if (presetNames.length === 0) {
			ctx.ui.notify(
				"No presets defined. Add presets to ~/.flame/agent/presets.json or .flame/presets.json",
				"warning",
			);
			return;
		}
