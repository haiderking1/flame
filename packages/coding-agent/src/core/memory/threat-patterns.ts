...
const PATTERNS: PatternSpec[] = [
	{
		pattern: "ignore\\s+(?:\\w+\\s+)*(previous|all|above|prior)\\s+(?:\\w+\\s+)*instructions",
		id: "prompt_injection",
		scope: "all",
	},
	{ pattern: "system\\s+prompt\\s+override", id: "sys_prompt_override", scope: "all" },
	{
		pattern: "disregard\\s+(?:\\w+\\s+)*(your|all|any)\\s+(?:\\w+\\s+)*(instructions|rules|guidelines)",
		id: "disregard_rules",
		scope: "all",
	},
	{
	...
	{
		pattern: "(respond|answer|reply)\\s+without\\s+(?:\\w+\\s+)*(restrictions|limitations|filters|safety)",
		id: "remove_filters",
		scope: "context",
	},
	{
		pattern: "you\\s+have\\s+been\\s+(?:\\w+\\s+)*(updated|upgraded|patched)\\s+to",
		id: "fake_update",
		scope: "context",
	},
	...
	{
		pattern: "never\\s+(?:\\w+\\s+)*(?:create|write)\\s+(?:\\w+\\s+)*(?:script|file)\\s+(?:\\w+\\s+)*disk",
		id: "anti_forensic_disk",
		scope: "context",
	},
	{
		pattern: "unset\\s+\\w*(?:CLAUDE|CODEX|HERMES|FLAME|AGENT|OPENAI|ANTHROPIC)\\w*",
		id: "env_var_unset_agent",
		scope: "context",
	},

	{
		pattern: "\\b(?:praxis|cobalt\\s*strike|sliver|havoc|mythic|metasploit|brainworm)\\b",
	...
	{
		pattern:
			"(include|output|print|share)\\s+(?:\\w+\\s+)*(conversation|chat\\s+history|previous\\s+messages|full\\s+context|entire\\s+context)",
		id: "context_exfil",
		scope: "strict",
	},
	...
	{
		pattern:
			"(update|modify|edit|write|change|append|add\\s+to)\\s+.*(?:AGENTS\\.md|CLAUDE\\.md|\\.cursorrules|\\.clinerules)",
		id: "agent_config_mod",
		scope: "strict",
	},
	{
		pattern: "(update|modify|edit|write|change|append|add\\s+to)\\s+.*\\.flame/(config\\.yaml|SOUL\\.md)",
		id: "flame_config_mod",
		scope: "strict",
	},
