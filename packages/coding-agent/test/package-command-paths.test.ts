...
describe("package commands", () => {
	...
	beforeEach(() => {
		tempDir = join(tmpdir(), `flame-package-commands-${Date.now()}-${Math.random().toString(36).slice(2)}`);
		...
		originalPiPackageDir = process.env.FLAME_PACKAGE_DIR;
	...
	it("shows install subcommand help", async () => {
		...
		try {
			...
			expect(stdout).toContain("flame install <source> [-l]");
	...
	it("shows a friendly error for unknown install options", async () => {
		...
		try {
			...
			expect(stderr).toContain('Use "flame --help" or "flame install <source> [-l]".');
	...
	it("shows a friendly error for missing install source", async () => {
		...
		try {
			...
			expect(stderr).toContain("Usage: flame install <source> [-l]");
	...
	it("uses global npmCommand and current package name for forced self updates without checking the api", async () => {
		...
		mkdirSync(join(projectDir, ".flame"), { recursive: true });
		writeFileSync(
			fakeNpmPath,
