...
describe("getThemeExportColors", () => {
	...
	beforeEach(() => {
		...
		previousAgentDir = process.env.FLAME_CODING_AGENT_DIR;
		process.env.FLAME_CODING_AGENT_DIR = join(tempRoot, "agent");
		mkdirSync(join(process.env.FLAME_CODING_AGENT_DIR, "themes"), { recursive: true });
	});
	...
	afterEach(() => {
		...
		if (previousAgentDir === undefined) {
			...
			process.env.FLAME_CODING_AGENT_DIR = previousAgentDir;
		}
	});
	...
	it("resolves export variable references using the same syntax as colors", () => {
		...
		writeFileSync(
			join(process.env.FLAME_CODING_AGENT_DIR!, "themes", "custom-export-vars.json"),
	...
	it("resolves recursive vars and converts 256-color export values to hex", () => {
		...
		writeFileSync(
			join(process.env.FLAME_CODING_AGENT_DIR!, "themes", "custom-export-recursive.json"),
			JSON.stringify(customTheme, null, 2),
		);
