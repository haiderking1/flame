...
> **Placement for /reload:** Put extensions in `~/.flame/agent/extensions/` (global) or `.flame/extensions/` (project-local) for auto-discovery. Use `pi -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
Create `~/.flame/agent/extensions/my-extension.ts`:

```typescript
import type { ExtensionAPI } from "@earendil-works/flame-coding-agent";
...
| Location | Scope |
|----------|-------|
| `~/.flame/agent/extensions/*.ts` | Global (all projects) |
| `~/.flame/agent/extensions/*/index.ts` | Global (subdirectory) |
| `.flame/extensions/*.ts` | Project-local |
| `.flame/extensions/*/index.ts` | Project-local (subdirectory) |
...
| Package | Purpose |
|---------|---------|
| `@earendil-works/flame-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@earendil-works/flame-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@earendil-works/flame-tui` | TUI components for custom rendering |

npm dependencies work too. Add a `package.json` next to your extension (or in a parent directory), run `npm install`, and imports from `node_modules/` are resolved automatically.

For distributed pi packages installed with `flame install` (npm or git), runtime deps must be in `dependencies`. Package installation uses production installs (`npm install --omit=dev`) by default, so `devDependencies` are not available at runtime; when `npmCommand` is configured, git packages use plain `install` for compatibility with wrappers.
...
```typescript
import { isToolCallEventType } from "@earendil-works/flame-coding-agent";
...
```typescript
import { isBashToolResult } from "@earendil-works/flame-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@earendil-works/flame-coding-agent";
...
```typescript
import { SessionManager } from "@earendil-works/flame-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@earendil-works/flame-tui";
...
```typescript
import { withFileMutationQueue } from "@earendil-works/flame-coding-agent";
...
**Important:** Use `StringEnum` from `@earendil-works/flame-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@earendil-works/flame-coding-agent";
...
```typescript
import { createBashTool } from "@earendil-works/flame-coding-agent";
...
} from "@earendil-works/flame-coding-agent";
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/earendil-works/flame-mono/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { keyHint } from "@earendil-works/flame-coding-agent";
...
```typescript
import { Text, Component } from "@earendil-works/flame-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@earendil-works/flame-coding-agent";
import { matchesKey } from "@earendil-works/flame-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@earendil-works/flame-coding-agent";
