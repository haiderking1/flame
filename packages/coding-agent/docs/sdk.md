...
```bash
npm install @earendil-works/flame-coding-agent
```
...
```typescript
import { createAgentSession, SessionManager } from "@earendil-works/flame-coding-agent";
...
The `Agent` class (from `@earendil-works/flame-agent-core`) handles the core LLM interaction. Access it via `session.agent`.
...
const { session } = await createAgentSession({
  ...
  agentDir: "~/.flame/agent", // default (expands ~)
});
```

`cwd` is used by `DefaultResourceLoader` for:
- Project extensions (`.flame/extensions/`)
- Project skills:
  - `.flame/skills/`
  - `.agents/skills/` in `cwd` and ancestor directories (up to git repo root, or filesystem root when not in a repo)
- Project prompts (`.flame/prompts/`)
...
- Global skills:
  - `skills/` under `agentDir` (for example `~/.flame/agent/skills/`)
  - `~/.agents/skills/`
...
import { AuthStorage, ModelRegistry } from "@earendil-works/flame-coding-agent";
...
// Default: uses ~/.flame/agent/auth.json and ~/.flame/agent/models.json
const authStorage = AuthStorage.create();
const modelRegistry = ModelRegistry.create(authStorage);
...
```typescript
import { createAgentSession, DefaultResourceLoader } from "@earendil-works/flame-coding-agent";
...
```typescript
import { createAgentSession } from "@earendil-works/flame-coding-agent";
...
import { createAgentSession, defineTool } from "@earendil-works/flame-coding-agent";
...
Extensions are loaded by the `ResourceLoader`. `DefaultResourceLoader` discovers extensions from `~/.flame/agent/extensions/`, `.flame/extensions/`, and settings.json extension sources.
...
```typescript
import { createEventBus, DefaultResourceLoader } from "@earendil-works/flame-coding-agent";
...
```typescript
import { createAgentSession, SettingsManager, SessionManager } from "@earendil-works/flame-coding-agent";
...
1. Global: `~/.flame/agent/settings.json`
2. Project: `<cwd>/.flame/settings.json`
